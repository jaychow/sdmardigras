...
function hero_background($atts, $content=null) {
	...
	if($slider_heading_option=='true' || $slider_icon_option=='true' || $slider_content_option=='true' || $coming_soon=='true' || $first_button_option=='true' || $notify_form_option=='true') {
		...
		if($first_button_option=='true'){
			//TODO: Move First Button Here

			// SD Mardi Gras Info
			$mardiGras = get_post( 674 )->post_content;

			$btn1ColSize = 'col-xs-12';
			$btn2ColSize = 'col-xs-6 col-sm-5 col-sm-offset-2';
			if($second_button_option == 'true') {
				$btn1ColSize = 'col-xs-6 col-sm-5';
				$btn2ColSize = 'col-xs-6 col-sm-5 col-sm-offset-2';
			}

			$contentOut .='<div class="container align-center ticket-options">';

				$contentOut.='<div class="row">'; // Row Start
					$contentOut.='<div class="'.$btn1ColSize.' ticket-option">';
						// BTN 1 Social Icons
						$contentOut.='<div class="social-icons">';
							$contentOut .= '<a href="'.$btn1_social_link_1.'" target="_blank"><i class="fa fa-calendar fa-2x"></i></a> <a href="'.$btn1_social_link_2.'" target="_blank"><i class="fa fa-facebook-official fa-2x"></i></a>';
						$contentOut.='</div>';
						...
						$contentOut.='<div>';
							$contentOut.='<a class="first-button button button-'.$button_size.' radius-'.$button_border_radius.$btnEffect.' wow animated fadeIn" data-wow-duration="0.5s" data-wow-delay="1.5s"'.mtheme_html_content($register_link).'>';
							$contentOut.=mtheme_html($register_title);
							$contentOut.='</a>';
				...
				if($second_button_option=='true') {
					...
					$contentOut.='<div class="'.$btn2ColSize.' ticket-option">';
						$contentOut.='<div class="social-icons">';
							$contentOut .= '<a href="https://www.facebook.com/events/1231074680279337/" target="_blank"><i class="fa fa-calendar fa-2x"></i></a> <a href="http://www.facebook.com/GaslampMardiGras" target="_blank"><i class="fa fa-facebook-official fa-2x"></i></a>';
						$contentOut.='</div>';
						...
						$contentOut.='<div>';
							$contentOut.='<a class="second-button button button-'.$second_button_size.' radius-'.$second_button_border_radius.$btnEffect.' wow animated fadeIn" data-wow-duration="0.5s" data-wow-delay="1.5s"'.mtheme_html_content($second_register_link).'>';
								$contentOut.=mtheme_html($second_register_title);
							$contentOut.='</a>';
						$contentOut.='</div>';
...
function post_background($atts, $content=null) {
	...
	if($type=='slider') {
		...
		if($query->have_posts()) {
			...
			{
				$post_overlay_image=MthemeCore::getOption("post_overlay_image",CHILD_URI.'site/img/backgrounds/bg_pattern.png');
			}
			...
			$out.='</div>';
			if(MthemeCore::getOption("post_overlay_active",'true')=='true'){
	...
	elseif($type=='vimeo') {
		...
		if(empty($overlay_image))
		{
			$overlay_image=MthemeCore::getOption("post_overlay_image",CHILD_URI.'site/img/backgrounds/bg_pattern.png');
		}
